...
- 首先，要出现熊市价格反转，或称之为上涨转下跌。为了形成熊市价格反转，要求至少有六根K线，其中**第五根K线的收盘价比第一根K线的收盘价高（Price Flip）**，第六根K线的收盘价，比第二根K线的收盘价低，那么就形成了熊市价格反转。这第六根K线，同时也是所谓的TD买入结构的第一根K线。  
...
## Countdown(TD计数): 
Countdown分为序列计数和组合计数。序列型和组合型拥有相同的Setup序列。“序列型”策略在趋势市场和盘整市场中都能做出有效反应，而“组合型”策略在趋势市场中反应更为灵敏，在盘整时期则表现较为迟缓。  
...
TD组合的严格计数条件为：  
- K线X的收盘价，要求小于K线X-2的最低价；  
- K线X的最低价，要求小于K线X-1的最低价；  
- K线X的收盘价，要求小于K线X-1的收盘价；  
- K线X的收盘价，要求小于**前一个计数K线**的收盘价。  

> 注：前一个计数K线，指符合计数条件的K线，而不是当前K线的前一根K线。

```python
# Buy Combo Countdown Condition
BAR[t].Close < BAR[t-2].Low and 
BAR[t].Low < BAR[t-1].Low and 
BAR[t].Close < BAR[t-1].Close and 
BAR[t].Close < BAR[Countdown.last()-1].Close
```
...
- 如果出现新的同方向的TD结构，则有可能形成两个TD买入计数嵌套的情况。判读的核心逻辑为：如果出现新的同方向更强的TD结构，则以新结构为准，如果不够强，则保留原计数，如果过于极端，也保留原结构。具体可一按照以下三种情况处理：
  - 如果**TD买入结构2**的收盘区间在**TD买入结构1**的真实波幅之内，并且**TD买入结构2**的价格极值也在**TD买入结构1**的真实波幅之内，那么继续买入计数1，不重新计数。  
  - 如果[**TD买入结构2**](#note2)的[真实波幅](#note2 "实际最高价 - 实际最低价")大于等于[**TD买入结构1**](#note2)的真实波幅，但是又不超过其1.618倍，则取消原计数，重新计数。  
  - 如果**TD买入结构2**的真实波幅超过**TD买入结构1**的真实波幅的1.618倍，那么**TD买入结构2**视为市场波动异常，忽略取消计数，保留原TD买入计数，不重新计数。  

<a id="recycle"></a>

**Recycle(循环计数)**：如果**TD买入结构2**未触发取消计数，并延伸到**18根**K线，那么**正在计数**的countdown会被Recycle。Recycle虽然立即发生，但字符“R”将标记在旧的计数13处，表示循环计数，同时说明下跌趋势非常强列。字符R的标记位置不影响新的countdown的计数。新的Countdown从Recycle的第1天开始计数。
...
### Setup的使用(以买入为例)
**买入条件**：
- 构成[完美setup](#perfectsetup)
- TD买入结构内的任何一根K线的收盘价，都要在之前的TD卖出结构趋势支撑线之上。
- TD买入结构的第九根K线，要非常靠近之前的TD卖出结构趋势支撑线

**买入价位**：（激进的买入点）  
- 在setup[9]这根K线的收盘价买入

**止损价位**：
- 找到TD买入结构的实际最低价所对应的K线；
- 将该K线的实际最低价减去其真实波幅，就得到止损价位。

> 注：真实波幅=|最高价-最低价|与|最高价-昨收|以及|昨收-最低价|三者之间的最大值。

**目标价位**：  
- TD买入结构的第九根K线的收盘价与目标价为的价差，是其与止损位的价差的1.5倍以上

### Countdown的使用(以买入为例) 

**买入价位**：（以下择1即可）  
- 在计数13这根K线的收盘价入场（积极策略）  
- 出现以下[***买入信号***](#buy_signal)当天的收盘价入场（保守策略）
...
**止损价位**：
- 找到TD计数的实际最低价所对应的K线，计算该K线的实际最高价与实际最低价之差，从该K线的实际最低价中减去上述差值，即得到止损价位

**目标价位**：  
- 买入价位与目标价位的价差，是其与止损位的价差的1.5倍以上
...
- [TradingView DeMARK 9 13 TD DESCRIPTION](https://cn.tradingview.com/script/gVMuxasg-DeMARK-9-13/)
