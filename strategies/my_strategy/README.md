...
- [x] 对所有股票都进行回测，分别分析整个市场的胜率和对每只股票的胜率以及收益
- [x] tick级别的止损交易策略，止损的设置

- [x] 前复权定义整理至论文中，讲解全量更新
- [x] 寻找回测方法相关论文，引用方法

- [x] 抽取5支股票进行5年的回测，验证simtradelab和PTrade的一致性，并在论文中做相关的补充
- [x] 系统分层架构表（表4-1）能否结合图的方式表示
- [x] 4.8反复提到的“撮合口径”什么意思？
- [ ] 减少人名，以问题和研究脉络组织文献，以作者名标记关键贡献，以引用角标支撑观点
- [x] “市场范围：A股主板、创业板、科创板”改为全部股票的表述
- [x] 股票支数的修改，5510不是实际的股票支数，并在论文中表明筛选条件
- [x] 5.3中方向收益率公式中的P是什么含义？猜测表示价格，i，t作为共同坐标表示i事件对应的股票在t日的收盘价。应当在论文中给出明确解释。
- [ ] 目前的交易对完美countdown的处理，配置是不需要完美countdown，也就是回测分析中，除非刚好在第13根K线构成完美countdown，否则其他延续形成的完美countdown无法被交易。对完美countdown的信号有效性分析会有影响，应该如何处理？
- [ ] 事件驱动的回测分析中，是否可以以胜率和盈亏比作为横纵轴，股票代码作为点，绘制散点图，分析个股适用性？
- [ ] 稳健性检测


- [x] 第一章command+f搜索“预留”字段，修改文本
- [x] 最后只写展望，不要写不足
...
- [ ] 图5-2名字中下划前有斜杠
- [x] 角标字体全部改成小四
- [x] 是否需要最大持仓天数限制？后期可配置开关，默认为不限制。
- [x] 事件驱动的回测结果还是未考虑分红和送股的，需要更新
- [ ] 附件中补充详细的实验配置参数
- [ ] 单次开仓0.1%的仓位，初始资金改为10,000,000元,没有最大持仓数量限制，但应该会自然限制，在论文中改动



- [x] 需求分析单独一章
- [ ] 章节层级结构再明确，设计和实现要区分
- [ ] 部属到PTrade中并在答辩前记录这段时间的模拟实盘效果
- [x] 论文目标应该是分析该指标在A股的成功率。不需要讨论信号的可靠性，写了也没关系。
- [ ] 对股票的分析应该是每只股票的全部历史胜率，便于在需要选择的时候有分析权重
- [x] 双引号有英文的,需要修改
- [x] 上标有些位置不对，像是[25],检查一下
- [x] 图片的字太小，内容紧凑点
- [x] 第六章列点描述，看起来清晰一点
- [x] 6.2标题
- [ ] 解释TD风险价位和目标价位的计算方法
