...
| `total_pnl`             | 总实现盈亏  | 所有完整交易 `pnl` 求和；新版策略中 `pnl = price_pnl + dividend_income` | 正数为盈利，负数为亏损；应结合初始资金和样本规模解读                           |
...
| `trade_total_pnl`    | 当前分组总盈亏   | 当前分组完整交易 `pnl` 求和；新版策略中包含价差收益和现金分红收益 | 正负方向比绝对大小更重要，因为各组样本数可能不同     |
