...
| 买入时间戳差异 | PTrade 回测记录为当日 `15:00:00`，SimTradeLab 记录为当日 `00:00:00` | 两个环境对日线撮合时间的标记方式不同，实际对应同一交易日 | 可接受，不构成买入交易日差异 |
| 买入成交价差异 | 买入价最大差异 0.0466 元，最大相对差异约 0.0349% | PTrade 回测成交价保留两位小数，SimTradeLab 保留更高精度并体现本地撮合口径 | 可接受，会传导至止盈价和 pnl，但幅度较小 |
...
根据表4-3 和表4-4，本文认为 SimTradeLab 与 PTrade 回测环境在信号层、事件研究层和完整交易生命周期层具有较好一致性。两套环境下 TD 事件编号集合完全一致，Setup 与 Countdown 的计数路径、完成日期和取消原因没有差异；事件研究样本完全一致，方向收益、最大有利波动和最大不利波动仅存在小数级差异；交易层的买入状态、买入交易日、卖出交易日、退出原因、止损价、卖出价和分红收益字段也保持一致。当前差异主要集中在三个方面：第一，两个环境对日线成交时间戳的表达不同，但对应同一交易日，不构成交易时机上的实质性偏移；第二，两个环境的成交价精度和撮合口径略有差别，导致买入价、入场金额、止盈价和 pnl 出现小幅差异；第三，持仓快照作为过程性输出，在个别开仓记录上存在记录时点差异，但完整交易生命周期记录和组合资产结果未因此发生实质偏离。
...
本文使用 SimTradeData 提供的 A 股日线数据，并通过 SimTradeLab 在本地完成回测。正式统计前先按照第四章的一致性验证方法对 PTrade 与 SimTradeLab 的关键输出进行抽样对比；通过一致性验证后，再使用 SimTradeLab 执行全市场长周期回测。回测区间设定为 2016 年 1 月 1 日至 2025 年 12 月 31 日，覆盖主板、创业板和科创板中具备有效日线数据的股票。
