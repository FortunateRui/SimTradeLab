...
就本文当前回测研究而言，上述差异未改变 TD 信号样本、事件收益样本、交易方向、退出路径和组合层结果，且单笔收益差异相对于组合资金规模较小。因此，SimTradeLab 可以作为本文全市场长周期回测的有效本地替代环境。后续在正式填入回测结果时，本文以 `td_events.csv`、`event_outcomes.csv` 和 `trade.csv` 作为主要统计依据，以 `position_snapshot.csv` 作为辅助检查文件，不将过程性快照中的暂态记录差异作为策略表现差异。
