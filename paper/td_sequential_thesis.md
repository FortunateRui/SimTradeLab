...
由于本文将 SimTradeLab 作为大规模回测的主要执行环境，仅说明其运行速度更快是不充分的，还需要验证本地模拟环境与 PTrade 环境在关键行为上的一致性。本文采用小样本一致性验证方法：选取 600519.SH、000858.SZ、601318.SH、600036.SH 和 000651.SZ 五只样本股票，将同一份策略代码、相同配置、相同复权方式和相同交易成本参数分别放入 PTrade 回测环境与 SimTradeLab 本地模拟环境中运行，然后比较行情输入、TD 信号输出、交易触发、成交持仓和组合资产等结果。

本节比较的数据来自 PTrade 回测环境与 SimTradeLab 本地模拟环境。两组日志均出现“回测近似成交”记录，说明当前对比样本采用的是同一类日线级回测撮合逻辑。因此，一致性验证的重点在于两个回测环境对历史行情、信号计算、交易日期、成交价和持仓结果的处理是否一致。

表4-3  本地模拟环境一致性验证结果表

| 验证层次 | 对比内容 | 合格口径 | 验证结果 |
| ---- | ---- | ---- | ---- |
| 行情输入 | 交易日历、OHLCV、前复权价格和统计字段 | 价格差异不影响 TD 比较关系 | 价格类字段最大差异约 0.01，20 日成交额均值存在小数精度差异，属于数据精度和四舍五入差异 |
| 信号输出 | Setup 完成日、Countdown 计数路径、Countdown 完成日、取消原因 | 信号日期、方向和计数结果完全一致 | `td_events.csv` 均为 341 条事件，事件编号集合完全一致，类别字段无差异 |
| 事件表现 | 事件窗口、到期日、方向收益、MFE 和 MAE | 事件样本一致，指标差异很小 | `event_outcomes.csv` 均为 189 条记录，事件键完全一致，方向收益最大差异约 0.000087 |
| 交易触发 | 买入日期、买入拒绝原因、卖出日期、退出原因 | 可执行事件和不可执行事件判断一致 | `trade.csv` 均为 14 条交易生命周期记录，`NORMAL`、取消原因、买入状态和卖出原因完全一致 |
| 成交与持仓 | 成交价格、成交数量、止损价、止盈价、PnL | 差异来自撮合细节、时间戳和价格精度 | 买入数量、止损价、卖出价完全一致；买入价最大差异 0.0466 元，最大相对差异约 0.0349% |
| 组合结果 | 持仓快照、现金余额和总资产 | 资产差异不影响回测结论 | 持仓开平仓顺序一致，组合总资产最大差异约 0.48 元，相对于百万级初始资金可以忽略 |

表4-4  本地模拟环境差异来源分析表

| 差异类型 | 数据表现 | 原因分析 | 是否可接受 |
| ---- | ---- | ---- | ---- |
| 价格精度差异 | 000858.SZ 部分前复权价格相差约 0.01 元 | 两个环境对行情字段的小数保留位和浮点计算精度不同 | 可接受，未改变任何 Setup 或 Countdown 计数结果 |
| 买入时间戳差异 | PTrade 回测记录为 `15:00:00`，SimTradeLab 记录为 `00:00:00` | 两个环境对日线撮合时间的标记方式不同，实际对应同一交易日 | 可接受，属于时间戳表达差异，不是跨日买入差异 |
| 买入成交价差异 | 买入价最大差异 0.0466 元，最大相对差异约 0.0349% | PTrade 回测近似成交价格保留两位小数，SimTradeLab 保留更高精度并体现本地撮合口径 | 可接受，会传导至入场金额、止盈价和 PnL，但幅度很小 |
| 止损触发差异 | 三笔 STOP_LOSS 的卖出日期、止损价和卖出价均一致 | 两个环境在本次样本中均按回测撮合逻辑判断止损 | 可接受，止损路径一致 |
| 损益与资产差异 | 单笔 PnL 最大差异约 33.6 元，组合总资产最大差异约 0.48 元 | 主要由买入价精度差异传导形成，平仓价和交易路径一致 | 可接受，不影响策略适用性分析结论 |

根据表4-3 和表4-4，本文认为 SimTradeLab 与 PTrade 回测环境在信号层和交易路径层具有较好一致性。两套环境下 TD 事件编号集合完全一致，Setup 与 Countdown 的计数路径、完成日期和取消原因没有差异；交易层的买入数量、买入日、卖出日、卖出原因、止损价和卖出价也保持一致。当前差异主要来自两个方面：第一，两个环境对日线成交时间戳的表达不同，PTrade 回测将买入完成记录为当日 `15:00:00`，SimTradeLab 将日线事件记录为当日 `00:00:00`，但二者对应同一交易日，并不构成交易时机上的实质性偏移；第二，两个环境的成交价精度和撮合口径略有差别，导致买入价、入场金额、止盈价和 PnL 出现小幅差异。

就本文当前回测研究而言，差异集中在时间戳标记、价格精度和由买入价传导的损益小数差异，未改变信号样本、交易方向和退出路径，且相对于百万级初始资金规模非常小，可以认为处于可接受范围内。SimTradeLab 因而可以作为本文全市场长周期回测的有效本地替代环境。
