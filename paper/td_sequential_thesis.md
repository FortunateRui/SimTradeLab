...
由于本文将 SimTradeLab 作为大规模回测的主要执行环境，仅说明其运行速度更快是不充分的，还需要验证本地模拟环境与 PTrade 环境在关键行为上的一致性。本文采用小样本一致性验证方法：在全市场回测前，选取若干只具有代表性的股票和交易区间，将同一份策略代码、相同配置、相同复权方式和相同交易成本参数分别放入 PTrade 与 SimTradeLab 中运行，然后比较行情输入、TD 信号输出、交易触发和资产变化等结果。

一致性验证不要求覆盖 5000 多只股票和 10 年样本，而是重点覆盖可能造成平台差异的场景。样本选择包括普通连续交易股票、存在停牌的股票、出现涨跌停约束的股票、发生除权除息的股票以及流动性较低的股票。验证时先关闭滑点和交易成本，检查信号层和委托触发是否一致；再开启交易成本，检查成交金额、持仓数量、现金余额和组合净值变化是否在可解释误差范围内。

表4-3  本地模拟环境一致性验证结果表

| 验证层次 | 对比内容 | 合格口径 | 验证结果 |
| ---- | ---- | ---- | ---- |
| 行情输入 | 交易日历、OHLCV、前复权价格、停牌和涨跌停状态 | 抽样日期字段一致，价格差异不超过 0.000001 | 【待补充】 |
| 信号输出 | Setup 完成日、Countdown 计数路径、Countdown 完成日、取消原因 | 信号日期、方向和计数结果完全一致 | 【待补充】 |
| 交易触发 | 买入日期、买入拒绝原因、卖出日期、退出原因 | 可执行事件和不可执行事件判断一致 | 【待补充】 |
| 成交与持仓 | 成交价格、成交数量、现金余额、持仓数量 | 无成本模式完全一致，含成本模式误差可解释 | 【待补充】 |
| 组合结果 | 逐日总资产、收益曲线、最大回撤 | 资产曲线差异仅来自撮合细节或四舍五入 | 【待补充】 |

根据表4-3，本文得到的本地模拟环境一致性验证结论为：【待补充】。若验证结果显示信号层完全一致、交易层差异均能由涨跌停顺延、成交价口径或四舍五入解释，则可以认为 SimTradeLab 能够作为本文大规模回测的有效替代环境；若出现无法解释的信号日期或计数路径差异，则需要优先修正本地模拟环境或策略适配代码，再进行全市场统计。
